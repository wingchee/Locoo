...
#if DEBUG
	#define CHECK_FOR_AFCACHE_PLIST() if([key isEqualToString:@"AFCache.plist"]) { \
		NSLog(@"AFCache.plist is a reserved key and can not be modified."); \
		return; }
#else
	#define CHECK_FOR_AFCACHE_PLIST() if([key isEqualToString:@"AFCache.plist"]) return;
#endif



static NSString* _AFCacheDirectory;

static inline NSString* AFCacheDirectory() {
	if(!_AFCacheDirectory) {
		NSString* cachesDirectory = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0];
		_AFCacheDirectory = [[[cachesDirectory stringByAppendingPathComponent:[[NSProcessInfo processInfo] processName]] stringByAppendingPathComponent:@"AFCache"] copy];
	}
	
	return _AFCacheDirectory;
}

static inline NSString* cachePathForKey(NSString* key) {
	return [AFCacheDirectory() stringByAppendingPathComponent:key];
}

static AFCache* __instance;
...
- (id)init {
	if((self = [super init])) {
		NSDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:cachePathForKey(@"AFCache.plist")];
		...
		[[NSFileManager defaultManager] createDirectoryAtPath:AFCacheDirectory() 
...
- (void)saveCacheDictionary {
	@synchronized(self) {
		[cacheDictionary writeToFile:cachePathForKey(@"AFCache.plist") atomically:YES];
	}
}
